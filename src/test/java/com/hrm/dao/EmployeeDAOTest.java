...
@DisplayName("Unit Test: EmployeeDAO - 100% DAO Branch Coverage")
public class EmployeeDAOTest {
    ...
    private void runMethodsWithSetup(Connection mockCon, PreparedStatement mockPs, Statement mockStmt, ResultSet mockRs, boolean nullOrEmptyArgs) {
        try (MockedStatic<DBConnection> mockedDb = mockStatic(DBConnection.class)) {
            ...
            try {
                for (Constructor<?> ctor : EmployeeDAO.class.getDeclaredConstructors()) {
                    try {
                        ...
                        instance = (EmployeeDAO) ctor.newInstance(args);
                        if (instance != null) break;
                    } catch (Throwable t2) { /* ok */ }
                }
            } catch (Throwable t) { /* ok */ }

            for (Method m : EmployeeDAO.class.getDeclaredMethods()) {
            ...
            for (Class<?> inner : EmployeeDAO.class.getDeclaredClasses()) {
