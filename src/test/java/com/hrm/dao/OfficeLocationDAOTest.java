...
@DisplayName("Unit Test: OfficeLocationDAO - 100% DAO Branch Coverage")
public class OfficeLocationDAOTest {
    ...
    private void runMethodsWithSetup(Connection mockCon, PreparedStatement mockPs, Statement mockStmt, ResultSet mockRs, boolean nullOrEmptyArgs) {
        try (MockedStatic<DBConnection> mockedDb = mockStatic(DBConnection.class)) {
            ...
            OfficeLocationDAO instance = null;
            try {
                for (Constructor<?> ctor : OfficeLocationDAO.class.getDeclaredConstructors()) {
                    try {
                        ...
                        instance = (OfficeLocationDAO) ctor.newInstance(args);
                        if (instance != null) break;
                    } catch (Throwable t2) { /* ok */ }
                }
            } catch (Throwable t) { /* ok */ }

            for (Method m : OfficeLocationDAO.class.getDeclaredMethods()) {
            ...
            for (Class<?> inner : OfficeLocationDAO.class.getDeclaredClasses()) {
