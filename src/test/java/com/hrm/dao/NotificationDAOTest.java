...
@DisplayName("Unit Test: NotificationDAO - 100% DAO Branch Coverage")
public class NotificationDAOTest {
    ...
    private void runMethodsWithSetup(Connection mockCon, PreparedStatement mockPs, Statement mockStmt, ResultSet mockRs, boolean nullOrEmptyArgs) {
        try (MockedStatic<DBConnection> mockedDb = mockStatic(DBConnection.class)) {
            ...
            NotificationDAO instance = null;
            try {
                for (Constructor<?> ctor : NotificationDAO.class.getDeclaredConstructors()) {
                    try {
                        ...
                        instance = (NotificationDAO) ctor.newInstance(args);
                        if (instance != null) break;
                    } catch (Throwable t2) { /* ok */ }
                }
            } catch (Throwable t) { /* ok */ }

            for (Method m : NotificationDAO.class.getDeclaredMethods()) {
            ...
            for (Class<?> inner : NotificationDAO.class.getDeclaredClasses()) {
