...
@DisplayName("Unit Test: EmployeeAllowanceDAO - 100% DAO Branch Coverage")
public class EmployeeAllowanceDAOTest {
    ...
    private void runMethodsWithSetup(Connection mockCon, PreparedStatement mockPs, Statement mockStmt, ResultSet mockRs, boolean nullOrEmptyArgs) {
        try (MockedStatic<DBConnection> mockedDb = mockStatic(DBConnection.class)) {
            ...
            EmployeeAllowanceDAO instance = null;
            try {
                for (Constructor<?> ctor : EmployeeAllowanceDAO.class.getDeclaredConstructors()) {
                    try {
                        ...
                        instance = (EmployeeAllowanceDAO) ctor.newInstance(args);
                        if (instance != null) break;
                    } catch (Throwable t2) { /* ok */ }
                }
            } catch (Throwable t) { /* ok */ }

            for (Method m : EmployeeAllowanceDAO.class.getDeclaredMethods()) {
            ...
            for (Class<?> inner : EmployeeAllowanceDAO.class.getDeclaredClasses()) {
