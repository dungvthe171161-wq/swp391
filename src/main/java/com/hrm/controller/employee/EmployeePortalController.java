...
public class EmployeePortalController extends HttpServlet {
    ...
    private void handleLeaveCreate(int employeeId, HttpServletRequest request, HttpServletResponse response)
        ...
        if (success) {
            ...
            request.getSession().setAttribute(mailWarning == null ? "employeeSuccess" : "employeeError",
                    mailWarning == null
                            ? "Đã gửi đơn nghỉ phép và email bàn giao."
                            : "Đã lưu đơn nghỉ phép, nhưng chưa gửi được email bàn giao: " + mailWarning);
        } else {
            request.getSession().setAttribute("employeeError", "Không thể gửi đơn. Vui lòng thử lại.");
        }
        response.sendRedirect(request.getContextPath() + "/employee/leaves");
    }
    ...
    private String validateLeaveRequest(int employeeId, String leaveType, String leaveDetail,
        ...
        if (!VALID_LEAVE_DETAILS.contains(leaveDetail)) {
            return "Chi tiết nghỉ không hợp lệ.";
        }
        ...
        if (startDate.isBefore(today) || endDate.isBefore(today)) {
            return "Ngày nghỉ không được nằm trong quá khứ.";
        }
        if (endDate.isBefore(startDate)) {
            return "Đến ngày không được nhỏ hơn từ ngày.";
        }
        if (!isValidEmail(handoverEmail)) {
            return "Vui lòng nhập email người nhận bàn giao hợp lệ.";
        }
        if (handoverWork.isBlank()) {
            return "Vui lòng nhập nội dung công việc cần bàn giao.";
        }
        if (reason.isBlank()) {
    ...
    private String sendHandoverEmail(HttpServletRequest request, String handoverEmail, LocalDate startDate,
        ...
        try {
            ...
            String employeeName = currentEmployee != null ? currentEmployee.getFullName()
                    : currentUser != null ? currentUser.getUsername() : "Nhân viên BetterHR";
            String subject = "BetterHR - Thông tin bàn giao công việc khi nghỉ phép";
            String content = """
                Xin chao,
                ...
                Vui lòng kiểm tra và phối hợp xử lý công việc được bàn giao.
            ...
            return ex.getMessage() == null ? "Lỗi gửi email." : ex.getMessage();
        }
    }
