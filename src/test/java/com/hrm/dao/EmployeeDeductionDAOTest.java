...
@DisplayName("Unit Test: EmployeeDeductionDAO - 100% DAO Branch Coverage")
public class EmployeeDeductionDAOTest {
    ...
    private void runMethodsWithSetup(Connection mockCon, PreparedStatement mockPs, Statement mockStmt, ResultSet mockRs, boolean nullOrEmptyArgs) {
        try (MockedStatic<DBConnection> mockedDb = mockStatic(DBConnection.class)) {
            ...
            EmployeeDeductionDAO instance = null;
            try {
                for (Constructor<?> ctor : EmployeeDeductionDAO.class.getDeclaredConstructors()) {
                    try {
                        ...
                        instance = (EmployeeDeductionDAO) ctor.newInstance(args);
                        if (instance != null) break;
                    } catch (Throwable t2) { /* ok */ }
                }
            } catch (Throwable t) { /* ok */ }

            for (Method m : EmployeeDeductionDAO.class.getDeclaredMethods()) {
            ...
            for (Class<?> inner : EmployeeDeductionDAO.class.getDeclaredClasses()) {
