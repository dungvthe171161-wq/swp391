...
@DisplayName("Unit Test: ContractDocumentDAO - 100% DAO Branch Coverage")
public class ContractDocumentDAOTest {
    ...
    private void runMethodsWithSetup(Connection mockCon, PreparedStatement mockPs, Statement mockStmt, ResultSet mockRs, boolean nullOrEmptyArgs) {
        try (MockedStatic<DBConnection> mockedDb = mockStatic(DBConnection.class)) {
            ...
            ContractDocumentDAO instance = null;
            try {
                for (Constructor<?> ctor : ContractDocumentDAO.class.getDeclaredConstructors()) {
                    try {
                        ...
                        instance = (ContractDocumentDAO) ctor.newInstance(args);
                        if (instance != null) break;
                    } catch (Throwable t2) { /* ok */ }
                }
            } catch (Throwable t) { /* ok */ }

            for (Method m : ContractDocumentDAO.class.getDeclaredMethods()) {
            ...
            for (Class<?> inner : ContractDocumentDAO.class.getDeclaredClasses()) {
