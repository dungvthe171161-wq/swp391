...
@DisplayName("Unit Test: ChatbotRoleDataDAO - 100% DAO Branch Coverage")
public class ChatbotRoleDataDAOTest {
    ...
    private void runMethodsWithSetup(Connection mockCon, PreparedStatement mockPs, Statement mockStmt, ResultSet mockRs, boolean nullOrEmptyArgs) {
        try (MockedStatic<DBConnection> mockedDb = mockStatic(DBConnection.class)) {
            ...
            ChatbotRoleDataDAO instance = null;
            try {
                for (Constructor<?> ctor : ChatbotRoleDataDAO.class.getDeclaredConstructors()) {
                    try {
                        ...
                        instance = (ChatbotRoleDataDAO) ctor.newInstance(args);
                        if (instance != null) break;
                    } catch (Throwable t2) { /* ok */ }
                }
            } catch (Throwable t) { /* ok */ }

            for (Method m : ChatbotRoleDataDAO.class.getDeclaredMethods()) {
            ...
            for (Class<?> inner : ChatbotRoleDataDAO.class.getDeclaredClasses()) {
