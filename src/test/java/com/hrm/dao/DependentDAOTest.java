...
@DisplayName("Unit Test: DependentDAO - 100% DAO Branch Coverage")
public class DependentDAOTest {
    ...
    private void runMethodsWithSetup(Connection mockCon, PreparedStatement mockPs, Statement mockStmt, ResultSet mockRs, boolean nullOrEmptyArgs) {
        try (MockedStatic<DBConnection> mockedDb = mockStatic(DBConnection.class)) {
            ...
            DependentDAO instance = null;
            try {
                for (Constructor<?> ctor : DependentDAO.class.getDeclaredConstructors()) {
                    try {
                        ...
                        instance = (DependentDAO) ctor.newInstance(args);
                        if (instance != null) break;
                    } catch (Throwable t2) { /* ok */ }
                }
            } catch (Throwable t) { /* ok */ }

            for (Method m : DependentDAO.class.getDeclaredMethods()) {
            ...
            for (Class<?> inner : DependentDAO.class.getDeclaredClasses()) {
