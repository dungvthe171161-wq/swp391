...
@DisplayName("Unit Test: RoleDAO - 100% DAO Branch Coverage")
public class RoleDAOTest {
    ...
    private void runMethodsWithSetup(Connection mockCon, PreparedStatement mockPs, Statement mockStmt, ResultSet mockRs, boolean nullOrEmptyArgs) {
        try (MockedStatic<DBConnection> mockedDb = mockStatic(DBConnection.class)) {
            ...
            try {
                for (Constructor<?> ctor : RoleDAO.class.getDeclaredConstructors()) {
                    try {
                        ...
                        instance = (RoleDAO) ctor.newInstance(args);
                        if (instance != null) break;
                    } catch (Throwable t2) { /* ok */ }
                }
            } catch (Throwable t) { /* ok */ }

            for (Method m : RoleDAO.class.getDeclaredMethods()) {
            ...
            for (Class<?> inner : RoleDAO.class.getDeclaredClasses()) {
