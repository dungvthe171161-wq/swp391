...
@DisplayName("Unit Test: WorkScheduleDAO - 100% DAO Branch Coverage")
public class WorkScheduleDAOTest {
    ...
    private void runMethodsWithSetup(Connection mockCon, PreparedStatement mockPs, Statement mockStmt, ResultSet mockRs, boolean nullOrEmptyArgs) {
        try (MockedStatic<DBConnection> mockedDb = mockStatic(DBConnection.class)) {
            ...
            WorkScheduleDAO instance = null;
            try {
                for (Constructor<?> ctor : WorkScheduleDAO.class.getDeclaredConstructors()) {
                    try {
                        ...
                        instance = (WorkScheduleDAO) ctor.newInstance(args);
                        if (instance != null) break;
                    } catch (Throwable t2) { /* ok */ }
                }
            } catch (Throwable t) { /* ok */ }

            for (Method m : WorkScheduleDAO.class.getDeclaredMethods()) {
            ...
            for (Class<?> inner : WorkScheduleDAO.class.getDeclaredClasses()) {
