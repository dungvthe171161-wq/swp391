...
@DisplayName("Unit Test: RecruitmentDAO - 100% DAO Branch Coverage")
public class RecruitmentDAOTest {
    ...
    private void runMethodsWithSetup(Connection mockCon, PreparedStatement mockPs, Statement mockStmt, ResultSet mockRs, boolean nullOrEmptyArgs) {
        try (MockedStatic<DBConnection> mockedDb = mockStatic(DBConnection.class)) {
            ...
            RecruitmentDAO instance = null;
            try {
                for (Constructor<?> ctor : RecruitmentDAO.class.getDeclaredConstructors()) {
                    try {
                        ...
                        instance = (RecruitmentDAO) ctor.newInstance(args);
                        if (instance != null) break;
                    } catch (Throwable t2) { /* ok */ }
                }
            } catch (Throwable t) { /* ok */ }

            for (Method m : RecruitmentDAO.class.getDeclaredMethods()) {
            ...
            for (Class<?> inner : RecruitmentDAO.class.getDeclaredClasses()) {
