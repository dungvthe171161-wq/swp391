...
@DisplayName("Unit Test: DeductionTypeDAO - 100% DAO Branch Coverage")
public class DeductionTypeDAOTest {
    ...
    private void runMethodsWithSetup(Connection mockCon, PreparedStatement mockPs, Statement mockStmt, ResultSet mockRs, boolean nullOrEmptyArgs) {
        try (MockedStatic<DBConnection> mockedDb = mockStatic(DBConnection.class)) {
            ...
            DeductionTypeDAO instance = null;
            try {
                for (Constructor<?> ctor : DeductionTypeDAO.class.getDeclaredConstructors()) {
                    try {
                        ...
                        instance = (DeductionTypeDAO) ctor.newInstance(args);
                        if (instance != null) break;
                    } catch (Throwable t2) { /* ok */ }
                }
            } catch (Throwable t) { /* ok */ }

            for (Method m : DeductionTypeDAO.class.getDeclaredMethods()) {
            ...
            for (Class<?> inner : DeductionTypeDAO.class.getDeclaredClasses()) {
