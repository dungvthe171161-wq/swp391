...
@DisplayName("Unit Test: RolePermissionDAO - 100% DAO Branch Coverage")
public class RolePermissionDAOTest {
    ...
    private void runMethodsWithSetup(Connection mockCon, PreparedStatement mockPs, Statement mockStmt, ResultSet mockRs, boolean nullOrEmptyArgs) {
        try (MockedStatic<DBConnection> mockedDb = mockStatic(DBConnection.class)) {
            ...
            RolePermissionDAO instance = null;
            try {
                for (Constructor<?> ctor : RolePermissionDAO.class.getDeclaredConstructors()) {
                    try {
                        ...
                        instance = (RolePermissionDAO) ctor.newInstance(args);
                        if (instance != null) break;
                    } catch (Throwable t2) { /* ok */ }
                }
            } catch (Throwable t) { /* ok */ }

            for (Method m : RolePermissionDAO.class.getDeclaredMethods()) {
            ...
            for (Class<?> inner : RolePermissionDAO.class.getDeclaredClasses()) {
