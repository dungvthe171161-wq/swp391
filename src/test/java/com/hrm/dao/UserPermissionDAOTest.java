...
@DisplayName("Unit Test: UserPermissionDAO - 100% DAO Branch Coverage")
public class UserPermissionDAOTest {
    ...
    private void runMethodsWithSetup(Connection mockCon, PreparedStatement mockPs, Statement mockStmt, ResultSet mockRs, boolean nullOrEmptyArgs) {
        try (MockedStatic<DBConnection> mockedDb = mockStatic(DBConnection.class)) {
            ...
            UserPermissionDAO instance = null;
            try {
                for (Constructor<?> ctor : UserPermissionDAO.class.getDeclaredConstructors()) {
                    try {
                        ...
                        instance = (UserPermissionDAO) ctor.newInstance(args);
                        if (instance != null) break;
                    } catch (Throwable t2) { /* ok */ }
                }
            } catch (Throwable t) { /* ok */ }

            for (Method m : UserPermissionDAO.class.getDeclaredMethods()) {
            ...
            for (Class<?> inner : UserPermissionDAO.class.getDeclaredClasses()) {
