...
@DisplayName("Unit Test: CandidateProfileDAO - 100% DAO Branch Coverage")
public class CandidateProfileDAOTest {
    ...
    private void runMethodsWithSetup(Connection mockCon, PreparedStatement mockPs, Statement mockStmt, ResultSet mockRs, boolean nullOrEmptyArgs) {
        try (MockedStatic<DBConnection> mockedDb = mockStatic(DBConnection.class)) {
            ...
            CandidateProfileDAO instance = null;
            try {
                for (Constructor<?> ctor : CandidateProfileDAO.class.getDeclaredConstructors()) {
                    try {
                        ...
                        instance = (CandidateProfileDAO) ctor.newInstance(args);
                        if (instance != null) break;
                    } catch (Throwable t2) { /* ok */ }
                }
            } catch (Throwable t) { /* ok */ }

            for (Method m : CandidateProfileDAO.class.getDeclaredMethods()) {
            ...
            for (Class<?> inner : CandidateProfileDAO.class.getDeclaredClasses()) {
