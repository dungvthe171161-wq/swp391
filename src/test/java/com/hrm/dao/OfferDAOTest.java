...
@DisplayName("Unit Test: OfferDAO - 100% DAO Branch Coverage")
public class OfferDAOTest {
    ...
    private void runMethodsWithSetup(Connection mockCon, PreparedStatement mockPs, Statement mockStmt, ResultSet mockRs, boolean nullOrEmptyArgs) {
        try (MockedStatic<DBConnection> mockedDb = mockStatic(DBConnection.class)) {
            ...
            OfferDAO instance = null;
            try {
                for (Constructor<?> ctor : OfferDAO.class.getDeclaredConstructors()) {
                    try {
                        ...
                        instance = (OfferDAO) ctor.newInstance(args);
                        if (instance != null) break;
                    } catch (Throwable t2) { /* ok */ }
                }
            } catch (Throwable t) { /* ok */ }

            for (Method m : OfferDAO.class.getDeclaredMethods()) {
            ...
            for (Class<?> inner : OfferDAO.class.getDeclaredClasses()) {
