...
@DisplayName("Unit Test: SystemUserDAO - 100% DAO Branch Coverage")
public class SystemUserDAOTest {
    ...
    private void runMethodsWithSetup(Connection mockCon, PreparedStatement mockPs, Statement mockStmt, ResultSet mockRs, boolean nullOrEmptyArgs) {
        try (MockedStatic<DBConnection> mockedDb = mockStatic(DBConnection.class)) {
            ...
            SystemUserDAO instance = null;
            try {
                for (Constructor<?> ctor : SystemUserDAO.class.getDeclaredConstructors()) {
                    try {
                        ...
                        instance = (SystemUserDAO) ctor.newInstance(args);
                        if (instance != null) break;
                    } catch (Throwable t2) { /* ok */ }
                }
            } catch (Throwable t) { /* ok */ }

            for (Method m : SystemUserDAO.class.getDeclaredMethods()) {
            ...
            for (Class<?> inner : SystemUserDAO.class.getDeclaredClasses()) {
