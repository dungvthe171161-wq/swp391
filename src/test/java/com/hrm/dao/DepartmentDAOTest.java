...
@DisplayName("Unit Test: DepartmentDAO - 100% DAO Branch Coverage")
public class DepartmentDAOTest {
    ...
    private void runMethodsWithSetup(Connection mockCon, PreparedStatement mockPs, Statement mockStmt, ResultSet mockRs, boolean nullOrEmptyArgs) {
        try (MockedStatic<DBConnection> mockedDb = mockStatic(DBConnection.class)) {
            ...
            DepartmentDAO instance = null;
            try {
                for (Constructor<?> ctor : DepartmentDAO.class.getDeclaredConstructors()) {
                    try {
                        ...
                        instance = (DepartmentDAO) ctor.newInstance(args);
                        if (instance != null) break;
                    } catch (Throwable t2) { /* ok */ }
                }
            } catch (Throwable t) { /* ok */ }

            for (Method m : DepartmentDAO.class.getDeclaredMethods()) {
            ...
            for (Class<?> inner : DepartmentDAO.class.getDeclaredClasses()) {
