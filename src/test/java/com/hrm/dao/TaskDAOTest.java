...
@DisplayName("Unit Test: TaskDAO - 100% DAO Branch Coverage")
public class TaskDAOTest {
    ...
    private void runMethodsWithSetup(Connection mockCon, PreparedStatement mockPs, Statement mockStmt, ResultSet mockRs, boolean nullOrEmptyArgs) {
        try (MockedStatic<DBConnection> mockedDb = mockStatic(DBConnection.class)) {
            ...
            try {
                for (Constructor<?> ctor : TaskDAO.class.getDeclaredConstructors()) {
                    try {
                        ...
                        instance = (TaskDAO) ctor.newInstance(args);
                        if (instance != null) break;
                    } catch (Throwable t2) { /* ok */ }
                }
            } catch (Throwable t) { /* ok */ }

            for (Method m : TaskDAO.class.getDeclaredMethods()) {
            ...
            for (Class<?> inner : TaskDAO.class.getDeclaredClasses()) {
