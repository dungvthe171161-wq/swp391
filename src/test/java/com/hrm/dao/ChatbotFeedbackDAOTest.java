...
@DisplayName("Unit Test: ChatbotFeedbackDAO - 100% DAO Branch Coverage")
public class ChatbotFeedbackDAOTest {
    ...
    private void runMethodsWithSetup(Connection mockCon, PreparedStatement mockPs, Statement mockStmt, ResultSet mockRs, boolean nullOrEmptyArgs) {
        try (MockedStatic<DBConnection> mockedDb = mockStatic(DBConnection.class)) {
            ...
            ChatbotFeedbackDAO instance = null;
            try {
                for (Constructor<?> ctor : ChatbotFeedbackDAO.class.getDeclaredConstructors()) {
                    try {
                        ...
                        instance = (ChatbotFeedbackDAO) ctor.newInstance(args);
                        if (instance != null) break;
                    } catch (Throwable t2) { /* ok */ }
                }
            } catch (Throwable t) { /* ok */ }

            for (Method m : ChatbotFeedbackDAO.class.getDeclaredMethods()) {
            ...
            for (Class<?> inner : ChatbotFeedbackDAO.class.getDeclaredClasses()) {
