...
@DisplayName("Unit Test: MailRequestDAO - 100% DAO Branch Coverage")
public class MailRequestDAOTest {
    ...
    private void runMethodsWithSetup(Connection mockCon, PreparedStatement mockPs, Statement mockStmt, ResultSet mockRs, boolean nullOrEmptyArgs) {
        try (MockedStatic<DBConnection> mockedDb = mockStatic(DBConnection.class)) {
            ...
            MailRequestDAO instance = null;
            try {
                for (Constructor<?> ctor : MailRequestDAO.class.getDeclaredConstructors()) {
                    try {
                        ...
                        instance = (MailRequestDAO) ctor.newInstance(args);
                        if (instance != null) break;
                    } catch (Throwable t2) { /* ok */ }
                }
            } catch (Throwable t) { /* ok */ }

            for (Method m : MailRequestDAO.class.getDeclaredMethods()) {
            ...
            for (Class<?> inner : MailRequestDAO.class.getDeclaredClasses()) {
