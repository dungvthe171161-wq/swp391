...
@DisplayName("Unit Test: ApplicationDAO - 100% DAO Branch Coverage")
public class ApplicationDAOTest {
    ...
    private void runMethodsWithSetup(Connection mockCon, PreparedStatement mockPs, Statement mockStmt, ResultSet mockRs, boolean nullOrEmptyArgs) {
        try (MockedStatic<DBConnection> mockedDb = mockStatic(DBConnection.class)) {
            ...
            ApplicationDAO instance = null;
            try {
                for (Constructor<?> ctor : ApplicationDAO.class.getDeclaredConstructors()) {
                    try {
                        ...
                        instance = (ApplicationDAO) ctor.newInstance(args);
                        if (instance != null) break;
                    } catch (Throwable t2) { /* ok */ }
                }
            } catch (Throwable t) { /* ok */ }

            for (Method m : ApplicationDAO.class.getDeclaredMethods()) {
            ...
            for (Class<?> inner : ApplicationDAO.class.getDeclaredClasses()) {
