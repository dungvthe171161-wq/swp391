...
@DisplayName("Unit Test: SystemLogDAO - 100% DAO Branch Coverage")
public class SystemLogDAOTest {
    ...
    private void runMethodsWithSetup(Connection mockCon, PreparedStatement mockPs, Statement mockStmt, ResultSet mockRs, boolean nullOrEmptyArgs) {
        try (MockedStatic<DBConnection> mockedDb = mockStatic(DBConnection.class)) {
            ...
            SystemLogDAO instance = null;
            try {
                for (Constructor<?> ctor : SystemLogDAO.class.getDeclaredConstructors()) {
                    try {
                        ...
                        instance = (SystemLogDAO) ctor.newInstance(args);
                        if (instance != null) break;
                    } catch (Throwable t2) { /* ok */ }
                }
            } catch (Throwable t) { /* ok */ }

            for (Method m : SystemLogDAO.class.getDeclaredMethods()) {
            ...
            for (Class<?> inner : SystemLogDAO.class.getDeclaredClasses()) {
