...
@DisplayName("Unit Test: LeaveDAO - 100% DAO Branch Coverage")
public class LeaveDAOTest {
    ...
    private void runMethodsWithSetup(Connection mockCon, PreparedStatement mockPs, Statement mockStmt, ResultSet mockRs, boolean nullOrEmptyArgs) {
        try (MockedStatic<DBConnection> mockedDb = mockStatic(DBConnection.class)) {
            ...
            LeaveDAO instance = null;
            try {
                for (Constructor<?> ctor : LeaveDAO.class.getDeclaredConstructors()) {
                    try {
                        ...
                        instance = (LeaveDAO) ctor.newInstance(args);
                        if (instance != null) break;
                    } catch (Throwable t2) { /* ok */ }
                }
            } catch (Throwable t) { /* ok */ }

            for (Method m : LeaveDAO.class.getDeclaredMethods()) {
            ...
            for (Class<?> inner : LeaveDAO.class.getDeclaredClasses()) {
