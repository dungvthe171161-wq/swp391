...
@DisplayName("Unit Test: DeptDashboardDAO - 100% DAO Branch Coverage")
public class DeptDashboardDAOTest {
    ...
    private void runMethodsWithSetup(Connection mockCon, PreparedStatement mockPs, Statement mockStmt, ResultSet mockRs, boolean nullOrEmptyArgs) {
        try (MockedStatic<DBConnection> mockedDb = mockStatic(DBConnection.class)) {
            ...
            DeptDashboardDAO instance = null;
            try {
                for (Constructor<?> ctor : DeptDashboardDAO.class.getDeclaredConstructors()) {
                    try {
                        ...
                        instance = (DeptDashboardDAO) ctor.newInstance(args);
                        if (instance != null) break;
                    } catch (Throwable t2) { /* ok */ }
                }
            } catch (Throwable t) { /* ok */ }

            for (Method m : DeptDashboardDAO.class.getDeclaredMethods()) {
            ...
            for (Class<?> inner : DeptDashboardDAO.class.getDeclaredClasses()) {
