...
@DisplayName("Unit Test: ChatbotReviewDAO - 100% DAO Branch Coverage")
public class ChatbotReviewDAOTest {
    ...
    private void runMethodsWithSetup(Connection mockCon, PreparedStatement mockPs, Statement mockStmt, ResultSet mockRs, boolean nullOrEmptyArgs) {
        try (MockedStatic<DBConnection> mockedDb = mockStatic(DBConnection.class)) {
            ...
            ChatbotReviewDAO instance = null;
            try {
                for (Constructor<?> ctor : ChatbotReviewDAO.class.getDeclaredConstructors()) {
                    try {
                        ...
                        instance = (ChatbotReviewDAO) ctor.newInstance(args);
                        if (instance != null) break;
                    } catch (Throwable t2) { /* ok */ }
                }
            } catch (Throwable t) { /* ok */ }

            for (Method m : ChatbotReviewDAO.class.getDeclaredMethods()) {
            ...
            for (Class<?> inner : ChatbotReviewDAO.class.getDeclaredClasses()) {
