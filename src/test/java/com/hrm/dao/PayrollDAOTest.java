...
@DisplayName("Unit Test: PayrollDAO - 100% DAO Branch Coverage")
public class PayrollDAOTest {
    ...
    private void runMethodsWithSetup(Connection mockCon, PreparedStatement mockPs, Statement mockStmt, ResultSet mockRs, boolean nullOrEmptyArgs) {
        try (MockedStatic<DBConnection> mockedDb = mockStatic(DBConnection.class)) {
            ...
            PayrollDAO instance = null;
            try {
                for (Constructor<?> ctor : PayrollDAO.class.getDeclaredConstructors()) {
                    try {
                        ...
                        instance = (PayrollDAO) ctor.newInstance(args);
                        if (instance != null) break;
                    } catch (Throwable t2) { /* ok */ }
                }
            } catch (Throwable t) { /* ok */ }

            for (Method m : PayrollDAO.class.getDeclaredMethods()) {
            ...
            for (Class<?> inner : PayrollDAO.class.getDeclaredClasses()) {
