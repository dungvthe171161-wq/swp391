...
@DisplayName("Unit Test: ChatbotFaqDAO - 100% DAO Branch Coverage")
public class ChatbotFaqDAOTest {
    ...
    private void runMethodsWithSetup(Connection mockCon, PreparedStatement mockPs, Statement mockStmt, ResultSet mockRs, boolean nullOrEmptyArgs) {
        try (MockedStatic<DBConnection> mockedDb = mockStatic(DBConnection.class)) {
            ...
            ChatbotFaqDAO instance = null;
            try {
                for (Constructor<?> ctor : ChatbotFaqDAO.class.getDeclaredConstructors()) {
                    try {
                        ...
                        instance = (ChatbotFaqDAO) ctor.newInstance(args);
                        if (instance != null) break;
                    } catch (Throwable t2) { /* ok */ }
                }
            } catch (Throwable t) { /* ok */ }

            for (Method m : ChatbotFaqDAO.class.getDeclaredMethods()) {
            ...
            for (Class<?> inner : ChatbotFaqDAO.class.getDeclaredClasses()) {
