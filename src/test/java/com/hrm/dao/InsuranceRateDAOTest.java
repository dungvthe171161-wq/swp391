...
@DisplayName("Unit Test: InsuranceRateDAO - 100% DAO Branch Coverage")
public class InsuranceRateDAOTest {
    ...
    private void runMethodsWithSetup(Connection mockCon, PreparedStatement mockPs, Statement mockStmt, ResultSet mockRs, boolean nullOrEmptyArgs) {
        try (MockedStatic<DBConnection> mockedDb = mockStatic(DBConnection.class)) {
            ...
            InsuranceRateDAO instance = null;
            try {
                for (Constructor<?> ctor : InsuranceRateDAO.class.getDeclaredConstructors()) {
                    try {
                        ...
                        instance = (InsuranceRateDAO) ctor.newInstance(args);
                        if (instance != null) break;
                    } catch (Throwable t2) { /* ok */ }
                }
            } catch (Throwable t) { /* ok */ }

            for (Method m : InsuranceRateDAO.class.getDeclaredMethods()) {
            ...
            for (Class<?> inner : InsuranceRateDAO.class.getDeclaredClasses()) {
