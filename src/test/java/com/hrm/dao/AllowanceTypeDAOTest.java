...
@DisplayName("Unit Test: AllowanceTypeDAO - 100% DAO Branch Coverage")
public class AllowanceTypeDAOTest {
    ...
    private void runMethodsWithSetup(Connection mockCon, PreparedStatement mockPs, Statement mockStmt, ResultSet mockRs, boolean nullOrEmptyArgs) {
        try (MockedStatic<DBConnection> mockedDb = mockStatic(DBConnection.class)) {
            ...
            AllowanceTypeDAO instance = null;
            try {
                for (Constructor<?> ctor : AllowanceTypeDAO.class.getDeclaredConstructors()) {
                    try {
                        ...
                        instance = (AllowanceTypeDAO) ctor.newInstance(args);
                        if (instance != null) break;
                    } catch (Throwable t2) { /* ok */ }
                }
            } catch (Throwable t) { /* ok */ }

            for (Method m : AllowanceTypeDAO.class.getDeclaredMethods()) {
            ...
            for (Class<?> inner : AllowanceTypeDAO.class.getDeclaredClasses()) {
