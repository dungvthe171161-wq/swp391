...
@DisplayName("Unit Test: PermissionDAO - 100% DAO Branch Coverage")
public class PermissionDAOTest {
    ...
    private void runMethodsWithSetup(Connection mockCon, PreparedStatement mockPs, Statement mockStmt, ResultSet mockRs, boolean nullOrEmptyArgs) {
        try (MockedStatic<DBConnection> mockedDb = mockStatic(DBConnection.class)) {
            ...
            try {
                for (Constructor<?> ctor : PermissionDAO.class.getDeclaredConstructors()) {
                    try {
                        ...
                        instance = (PermissionDAO) ctor.newInstance(args);
                        if (instance != null) break;
                    } catch (Throwable t2) { /* ok */ }
                }
            } catch (Throwable t) { /* ok */ }

            for (Method m : PermissionDAO.class.getDeclaredMethods()) {
            ...
            for (Class<?> inner : PermissionDAO.class.getDeclaredClasses()) {
